...
def create_gauge(start_angle=0, end_angle=180, step_size=5, min_value=0, max_value=40, reverse_labels=False, output_file='gauge.png',
    ...
    - min_value (int): The minimum value to be displayed on the gauge.
    - max_value (int): The maximum value to be displayed on the gauge.
    ...
    """
    # Calculate the number of ticks based on the min and max values and step size
    num_ticks = ((max_value - min_value) // step_size) + 1
    tick_labels = [f"{i}{label_suffix}" for i in range(min_value, max_value + step_size, step_size)]

    if reverse_labels:
        tick_labels.reverse()

    # Determine the angles for each tick
...
create_gauge(
    ...
    output_file='flexible_gauge.png',
    font_size=12, 
    font_weight='normal', 
    font_color='blue', 
    circle_linewidth=3, 
